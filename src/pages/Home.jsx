...
const slideshowPhotos = [
  { src: "/videos/ecebatch.jpeg", label: "St Johns College Of Engineering 🏫",  frame: "neon-purple" },
  { src: "/videos/ecedept1.jpeg", label: "ECE Department ✨",                    frame: "polaroid"    },
  { src: "/videos/ecedept2.jpeg", label: "Our Campus Life 🎓",                   frame: "neon-pink"   },
  { src: "/videos/ecedept3.jpeg", label: "Batch of 2022-2026 💖",                frame: "gradient"    },
  { src: "/videos/ecebatch.jpeg", label: "End of An Era 🌓",                     frame: "gold"        },
];
const frameConfig = {
...
function HeroSlideshow() {
  ...
  return (
    <div className="relative w-full h-[75vh] md:h-[85vh] flex flex-col items-center justify-center gap-5">
      ...
      <div className="relative w-full flex-1 flex items-center justify-center">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            key={current}
            className={`absolute inset-0 overflow-hidden ${cfg.shape} ${cfg.border} ${cfg.glow} ${cfg.extra || ""}`}
          ...
          >
            ...
            {/* LABEL */}
            <motion.div
              className={`absolute bottom-4 left-4 right-4 rounded-xl bg-gradient-to-r ${cfg.badge} backdrop-blur-md px-4 py-3`}
...
const StudentMosaicCard = memo(({ student, index }) => {
  ...
  return (
    <motion.div
      ...
      style={{ aspectRatio:"4/3", transform: hov?"translateY(-8px)":"translateY(0)", transition:"transform 0.25s ease", boxShadow: hov?`0 12px 36px ${g.glow}`:"none" }}
    ...
    <div className="bg-[#05020a] min-h-screen text-white">
      ...
      <section className="py-20 px-4">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4">
          {[["48","Students 🎓"],["13","Teachers 🙏"],["4","Years ✨"],["∞","Memories 💖"]].map(([v,l]) => (
      ...
      <section className="py-16 px-6 text-center max-w-2xl mx-auto">
        <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.9 }}>
          ...
          <p className="text-white/40 text-sm leading-relaxed">
            Strangers in 2022, family by 2026. A collective of dreamers building the future of ECE.
          </p>
        </motion.div>
      </section>
